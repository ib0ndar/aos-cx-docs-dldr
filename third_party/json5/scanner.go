...
func (s *scanner) eof() int {
	...
	if s.inBlockComment {
		s.err = &SyntaxError{"unterminated block comment", s.bytes}
		return scanError
	}
	if s.endTop {
		return scanEnd
...
func (s *scanner) pushParseState(p int) bool {
	limit := s.maxDepth
	if limit == 0 {
		limit = 1000
	}
	if len(s.parseState) >= limit {
		s.err = &SyntaxError{"JSON5 nesting limit exceeded", s.bytes}
		s.step = stateError
		return false
	}
...
func stateBeginValue(s *scanner, c byte) int {
	...
	if c != '/' {
		s.values++
		if s.maxValues > 0 && s.values > s.maxValues {
			s.err = &SyntaxError{"JSON5 value limit exceeded", s.bytes}
			s.step = stateError
			return scanError
		}
	}
	...
	case '{':
		...
		if !s.pushParseState(parseObjectKey) {
			return scanError
		}
		return scanBeginObject
	case '[':
		s.step = stateBeginValueOrEmpty
		if !s.pushParseState(parseArrayValue) {
			return scanError
		}
...
func stateInStringEsc(resume func(s *scanner, c byte) int) func(s *scanner, c byte) int {
	return func(s *scanner, c byte) int {
		...
		case 'x':
			...
			return scanContinue
		case '0':
			s.step = func(s *scanner, c byte) int {
				if c >= '0' && c <= '9' {
					return s.error(c, "after null escape")
				}
				s.step = resume
				return resume(s, c)
			}
...
func stateLeadingDot(s *scanner, c byte) int {
	if c >= '0' && c <= '9' {
		s.step = stateDot0
		return scanContinue
	}
	return s.error(c, "after leading decimal point")
}
