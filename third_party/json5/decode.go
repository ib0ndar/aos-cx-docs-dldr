...
func (d *decodeState) object(v reflect.Value) {
	...
	for {
		...
		key, valid := decodeKey(key)
		if !valid {
			d.error(&SyntaxError{"invalid JSON5 property name", int64(start + 1)})
		}
		if d.disallowDuplicateKeys && keys[string(key)] {
			d.error(&SyntaxError{"duplicate decoded object key: " + string(key), int64(start + 1)})
		}
...
func (d *decodeState) convertNumber(s string) (interface{}, error) {
	if !isValidNumber(s) {
		return nil, &SyntaxError{"invalid JSON5 number", int64(d.off)}
	}
	if d.useNumber {
		return Number(s), nil
	}
...
func (d *decodeState) objectInterface() map[string]interface{} {
	...
	for {
		...
		if _, exists := m[string(key)]; exists && d.disallowDuplicateKeys {
			d.error(&SyntaxError{"duplicate decoded object key: " + string(key), int64(start + 1)})
		}
...
func (d *decodeState) literalInterface() interface{} {
	...
	case '"', '\'': // string
		s, ok := unquote(item)
		if !ok {
			d.error(&SyntaxError{"invalid JSON5 string or unpaired surrogate", int64(start + 1)})
		}
		return s
	...
	for r < len(s) {
		...
		case c == '\\':
			...
			case 'x':
				...
				value, err := strconv.ParseUint(string(s[r+1:r+3]), 16, 8)
				if err != nil {
					return
				}
				w += utf8.EncodeRune(b[w:], rune(value))
				r += 3
			case '\n':
				r++
			case '\r':
			...
			case 'u':
				...
				if utf16.IsSurrogate(rr) {
					...
					// Do not silently lose a UTF-16 value in UTF-8 output.
					return
				}
				w += utf8.EncodeRune(b[w:], rr)
			}
