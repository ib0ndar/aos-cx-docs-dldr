...
if [ "$(uname -s)" != Darwin ] || [ "$(uname -m)" != arm64 ]; then
	echo "qpdf sidecar staging is supported only on macOS arm64" >&2
	exit 1
fi

SOURCE=$1
OUTPUT_ROOT=$2
DESTINATION="$OUTPUT_ROOT/qpdf-mac-arm64"
mkdir -p "$OUTPUT_ROOT"
"$AOSCX_RELEASE_TOOL" stage-tree --source "$SOURCE" --destination "$DESTINATION"
echo "Staged checksum-verified local qpdf sidecar at $DESTINATION"
